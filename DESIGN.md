...
| Slang's companion libraries (`slang-glslang`) not loadable from the payload on Windows | *resolved in 0.0.6* | Slang loads companions lazily and **by bare name**, which Windows resolves against the executable's directory, the system directories, the CWD and PATH — never against `slang-compiler.dll`'s own directory, and there is no `RPATH=$ORIGIN` to stand in for it (the macOS/Linux payloads have one, so they were never affected). A payload shipping `slang-glslang.dll` beside the compiler therefore still failed `SPIRV_ASM` / `spirv-opt` / `spirv-dis` / GLSL pass-through with `failed to load dynamic library 'slang-glslang'` unless the host happened to have a Slang install or the Vulkan SDK on PATH — which is exactly why it passed on a dev box and failed only on the Windows CI job, silently reading a *host* library instead of the pinned one. `IGlobalSession.create` now calls `setDownstreamCompilerPath` for the glslang-backed pass-throughs with `SlangLibrary.directory()`: the load stays lazy, and the pinned payload comes ahead of the host on every platform — the pinning §7's class-path case already promised. Reproduce a clean host by stripping Vulkan/Slang from PATH before running the suite. |
| Serialized module IR from another Slang release **aborts the process** | *resolved in 0.0.5* | `loadModuleFromIRBlob` does not fail — it calls `abort()` (Windows exit `0xC0000409`, no `hs_err`, no diagnostic), so a mismatch cannot be caught, only prevented. `Session.loadModuleFromIr` therefore pre-checks with `slang_loadModuleInfoFromIRBlob` (safe on exactly that IR) and throws `SlangCompileException` instead, per `ModuleIrCompatibilityTest`. The predicate is `ModuleInfo.moduleVersion() == GlobalSession.supportedModuleVersion()`, i.e. the **module** version, not the compiler version — two releases emitting the same module version interoperate. `supportedModuleVersion()` is observed by round-tripping a trivial module because Slang exposes the real range only through `slangc -get-supported-module-versions` (`CompilerOptionName.GetSupportedModuleVersions` has no C export), so a build that also reads *older* versions is treated conservatively: the cost is a needless source recompile, against a silent process kill. Worth revisiting if Slang ever exports the range. |
